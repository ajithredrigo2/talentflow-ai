...
export default function RecruitmentInbox() {
  ...
  return (
    <>
      ...
      {message && <div className="mb-4 rounded-xl border border-[#e6e9f2] bg-[#fafbfe] px-4 py-3 text-[13px] text-[#4a5470]">{message}</div>}
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_320px]">
            ...
            {FILTERS.map((f) => {
              ...
              return (
                <button
                  key={f}
                  onClick={() => setFilter(f)}
                  className={`badge transition ${filter === f ? 'bg-ink-900 text-white' : 'bg-[#f0f2f8] text-[#5a6480] hover:bg-[#e6e9f2]'}`}
                >
                ...
                {filtered.map((r) => (
                  <tr key={r.id} className="transition hover:bg-[#fafbfe]">
                    ...
                    <td className="td">
                      ...
                      <div className="text-[11.5px] text-[#8b93a9]">{r.fromEmail}</div>
                    ...
                    <td className="td">
                      {r.attachment ? (
                        r.attachment.quarantined ? (
                          ...
                          <span className="flex items-center gap-1 text-[11.5px] text-[#5a6480]">
                    ...
                    <td className="td">
                      ...
                      <div className="mt-0.5 text-[10.5px] text-[#9aa2b8]">{r.mailboxAddress}</div>
                    </td>
                    <td className="td">
                    ...
                    <td className="td w-32">{r.matchScore !== null ? <Meter value={r.matchScore} right={`${r.matchScore}%`} /> : <span className="text-[#c3c9d8]">—</span>}</td>
        ...
        <div className="space-y-4">
            ...
            <div className="space-y-2 p-3">
              {samples.map((s) => (
                <button
                  ...
                  disabled={!!busy}
                  className="block w-full rounded-lg border border-[#e6e9f2] px-3 py-2.5 text-left transition hover:border-brand-300 hover:bg-brand-50/40 disabled:opacity-50"
                >
                  ...
                  <div className="mt-0.5 font-mono text-[10.5px] text-[#8b93a9]">{s.subject}</div>
                  <p className="mt-1 text-[11.5px] leading-snug text-[#7a839c]">{s.note}</p>
                </button>
              ))}
            </div>
          </Card>
            ...
            <div className="divide-y divide-[#f2f4f9]">
              {connections.map((c) => (
                <div key={c.id} className="px-4 py-3">
                  ...
                  <div className="mt-0.5 text-[11px] text-[#9aa2b8]">{c.authMethod}{c.scopes.length ? ` · ${c.scopes.join(', ')}` : ''}</div>
                  <p className="mt-1 text-[11.5px] leading-snug text-[#7a839c]">{c.detail}</p>
                </div>
              ))}
            </div>
          </Card>
              ...
              {mailboxes.map((m) => (
                <div key={m.id} className="flex items-start justify-between gap-2 px-4 py-2.5">
                  <div>
                    ...
                    <div className="text-[11px] text-[#9aa2b8]">{m.label}</div>
                  </div>
                  <div className="text-right">
                    <div className="text-[12px] font-semibold text-ink-950">{m.receivedCount}</div>
                    <div className="text-[10px] text-[#9aa2b8]">{m.autoAcknowledge ? 'ack on' : 'ack off'}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          {liveConnections.length === 0 && (
            <div className="rounded-xl border border-[#e6e9f2] bg-[#fafbfe] p-3.5 text-[11.5px] leading-relaxed text-[#7a839c]">
