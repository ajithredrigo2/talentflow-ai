  ...
  return (
    <>
      ...
      <div className="mb-4 flex flex-wrap gap-1.5">
        <a href="/audit" className={`badge ${!sp.actor && !sp.severity ? 'bg-ink-900 text-white' : 'bg-[#f0f2f8] text-[#5a6480]'}`}>All</a>
        <a href="/audit?actor=ai" className={`badge ${sp.actor === 'ai' ? 'bg-ink-900 text-white' : 'bg-[#f0f2f8] text-[#5a6480]'}`}>AI agents</a>
        <a href="/audit?actor=human" className={`badge ${sp.actor === 'human' ? 'bg-ink-900 text-white' : 'bg-[#f0f2f8] text-[#5a6480]'}`}>Human decisions</a>
        <a href="/audit?severity=warning" className={`badge ${sp.severity === 'warning' ? 'bg-ink-900 text-white' : 'bg-[#f0f2f8] text-[#5a6480]'}`}>Warnings</a>
      </div>

      <Card>
        <Table head={['Timestamp', 'Actor', 'Role', 'Action', 'Entity', 'Detail', 'Severity']}>
          {logs.map((l) => (
            <tr key={l.id} className="transition hover:bg-[#fafbfe]">
              <td className="td whitespace-nowrap font-mono text-[11.5px] text-[#7a839c]">
                {new Date(l.at).toLocaleString('en-GB', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}
              </td>
              ...
              <td className="td font-mono text-[11.5px] text-[#4a5470]">{l.action}</td>
              <td className="td text-[#616b85]">{l.entity} <span className="text-[#9aa2b8]">{l.entityId.slice(0, 14)}</span></td>
              <td className="td max-w-lg text-[12px] text-[#616b85]">{l.detail}</td>
