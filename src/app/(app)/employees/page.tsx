      ...
      <div className="mb-4 grid gap-4 lg:grid-cols-3">
        ...
        <Card title="Filter by department">
          <div className="flex flex-wrap gap-1.5 p-4">
            <Link href="/employees" className={`badge ${!sp.dept ? 'bg-brand-600 text-white' : 'bg-[#f4f2f6] text-[#6b6377]'}`}>All ({employees.length})</Link>
            {departments.map((d) => (
              <Link key={d.id} href={`/employees?dept=${d.id}`} className={`badge ${sp.dept === d.id ? 'bg-brand-600 text-white' : 'bg-[#f4f2f6] text-[#6b6377]'}`}>{d.name} ({d.headcount})</Link>
            ))}
          </div>
        </Card>
      </div>
      ...
      <Card>
        <Table head={['Employee', 'Department', 'Level', 'Location', 'Tenure', 'Performance', 'Engagement', 'Retention', 'Status']}>
          {list.map((e) => {
            ...
            return (
              <tr key={e.id} className="transition hover:bg-[#fcfbfd]">
                <td className="td">
                  <div className="flex items-center gap-2.5">
                    <Avatar name={e.name} size={30} />
                    <div>
                      <Link href={`/employees/${e.id}`} className="font-medium text-ink-950 hover:text-brand-600">{e.name}</Link>
                      <div className="text-[11.5px] text-[#9892a2]">{e.title}</div>
                    </div>
                  </div>
                </td>
                <td className="td text-[#71697d]">{deptName(e.departmentId)}</td>
                <td className="td text-[#71697d]">{e.level}</td>
                <td className="td text-[#71697d]">{e.location}</td>
