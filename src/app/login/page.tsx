    ...
    <div className="grid min-h-screen lg:grid-cols-[1.05fr_1fr]">
      ...
      <div className="flex items-center justify-center bg-white px-6 py-12">
        <div className="w-full max-w-sm">
          ...
          <p className="mt-1.5 text-[13.5px] text-[#616b85]">Choose a demo role below, or enter credentials.</p>
          ...
          <div className="my-7 flex items-center gap-3 text-[11px] font-semibold uppercase tracking-wider text-[#9aa2b8]">
            ...
            {DEMO_ACCOUNTS.map((a) => (
              <button
                ...
                className={`w-full rounded-lg border px-3.5 py-2.5 text-left transition ${
                  email === a.email ? 'border-brand-400 bg-brand-50/60 ring-4 ring-brand-100' : 'border-[#e6e9f2] hover:border-[#cdd3e3] hover:bg-[#fafbfe]'
                }`}
              >
                <div className="flex items-center justify-between">
                  <span className="text-[13px] font-semibold text-ink-950">{a.label}</span>
                  <span className="font-mono text-[11px] text-[#8b93a9]">{a.email}</span>
                </div>
                <p className="mt-0.5 text-[11.5px] leading-snug text-[#7a839c]">{a.description}</p>
              </button>
            ))}
          </div>

          <p className="mt-6 text-center text-[11.5px] text-[#9aa2b8]">
            All demo accounts use <span className="font-mono text-[#616b85]">Demo@2026</span>. No real personal data is stored.
          </p>
        </div>
      </div>
    </div>
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="flex min-h-screen items-center justify-center text-[13px] text-[#616b85]">Loading…</div>}>
      <LoginForm />
    </Suspense>
  );
}
